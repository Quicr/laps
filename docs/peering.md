...
to establish a data forwarding-plane for them via the relay infrastructure. 
...
There are endless topologies to form the data forwarding-plane from publisher to one or more subscribers. The
general rule of thumb is to optimize with hub-and-spoke (aka Star) peering if possible and inject Via 
relays as needed to aggregate efficiently to reduce congestion/bandwidth.  Injecting Via relays are used
to steer data forwarding via different IP forwarding paths to avoid or workaround problems (e.g.,
