 ...
 *     See https://github.com/Quicr/qmsg/blob/main/doc/protocol.md for more
 * details.
 */
 ...
 *     encoded name for published messages. Subscribers subscribe using
 * bits/len, where name_length defines the number of significant bits.
 */
#define PUB_NAME_LEN 16
typedef struct {
  u_char data[PUB_NAME_LEN]; ///< Buffer for the spec struct, this must be the
                             ///< size of the packed spec
} __attribute__((__packed__, __aligned__(1))) PubName;

/**
 * QMsg named object
 * Defines the published name for qmsg over pub/sub. This is the binary
 *     encoded name from string
...
struct MsgHeaderFlags {
  u_char
      metrics : 1; ///< Set to indicate if metrics are included in the header.
  u_char reserved : 7; ///< Unused/remaining bits
} __attribute__((__packed__, __aligned__(1)));

/**
 * Defines the slow-relay message header. Every message header must start with
 * this. Data follows the header.
 */
struct MsgHeader {
...
/**
 * Defines the slow-relay publish message header. This follows the slow-relay
 * message header.
 */
struct MsgPubHeader {
...
/**
 * Defines the slow-relay subscribe message header. This follows the slow-relay
 * message header.
 */
struct MsgSubHeader {
...
int slowerRemote(SlowerRemote &remote, const char server[],
                 const uint16_t port = lapsDefaultPort);
...
int slowerPub(SlowerConnection &slower, const PubName &name, char buf[],
              int bufLen, SlowerRemote *remote = NULL,
              MsgHeaderMetrics *metrics = NULL);
int slowerAck(SlowerConnection &slower, const PubName &name,
              SlowerRemote *remote = NULL);
int slowerSub(SlowerConnection &slower, const PubName &name, int len,
              SlowerRemote *remote = NULL);
int slowerUnSub(SlowerConnection &slower, const PubName &name, int len,
                SlowerRemote *remote = NULL);

int slowerRecvPub(SlowerConnection &slower, MsgHeader *msgHeader, char buf[],
                  int bufSize, int *bufLen, MsgHeaderMetrics *metrics = NULL);
int slowerRecvAck(SlowerConnection &slower, PubName *name);
int slowerRecvMulti(SlowerConnection &slower, MsgHeader *msgHeader,
                    SlowerRemote *remote, int *len, char buf[], int bufSize,
                    int *bufLen, MsgHeaderMetrics *metrics = NULL);
