...
RUN apt-get update \
    && apt-get install -y make clang-19 libc++-19-dev libc++abi-19-dev llvm-19-dev openssl golang wget git cmake ca-certificates \
...
# Run layer
FROM debian:12-slim
RUN apt-get update && apt-get install -y libc++-19-dev libc++abi-19-dev bash
